...
export default async function DiscoveryCallDetailPage({ params }: { params: Promise<{ callId: string }> }) {
  ...
  return <div className="page">
    <div className="page-header"><div><Link className="sub" href="/calls">← Discovery calls</Link><div className="page-title" style={{ marginTop: 8 }}>{row.topic}</div><div className="page-sub">{row.started_at ? new Date(row.started_at).toLocaleString() : "Date unavailable"}{row.duration_minutes != null ? ` · ${row.duration_minutes} minutes` : ""}</div></div><div style={{ display: "flex", gap: 8, alignItems: "center" }}><span className={`badge ${statusClass}`}>{DISCOVERY_CALL_STATUS_LABELS[row.status]}</span>{row.recording_play_url && <a className="btn" href={row.recording_play_url} target="_blank" rel="noreferrer">Open recording ↗</a>}</div></div>
    ...
    <div className="two-col"><main>
      {brief ? <>
        <div className="card brief-hero"><h2>What George should build</h2><p className="brief-build-direction">{brief.what_to_build}</p><p>{brief.executive_summary}</p></div>
      ...
      {row.transcript && <details className="card transcript-card"><summary>Full transcript</summary><pre>{row.transcript}</pre></details>}
    </main><aside><DiscoveryCallActions callId={row.id} currentLeadId={row.lead_id} leads={leads ?? []} canRetry={row.status === "failed"} /><div className="card"><div className="card-title">Call details</div><div className="lead-detail-grid one-column"><div><span>Business</span><strong>{row.businesses?.name ?? "—"}</strong></div><div><span>Lead</span><strong>{row.leads?.name ?? "Not linked"}</strong></div><div><span>Host</span><strong>{row.host_email ?? "—"}</strong></div><div><span>Processed</span><strong>{row.processed_at ? new Date(row.processed_at).toLocaleString() : "—"}</strong></div></div></div></aside></div>
  </div>;
}

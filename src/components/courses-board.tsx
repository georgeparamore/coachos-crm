...
export function CoursesBoard({ initialCourses, initialModulesByCourse, initialLessonsByModule, enrollmentCountByCourse, coachId, initialCreate }: {
  ...
  async function addModule(courseId: string) {
    ...
    try {
      ...
      savedModulesRef.current[courseId] = [...(savedModulesRef.current[courseId] ?? []), data as CourseModule];
      savedLessonsRef.current[data.id] = [];
      setModuleTitle("");
  ...
  async function saveLesson(input: LessonInput) {
    ...
    if (lesson) {
      ...
      savedLessonsRef.current[moduleId] = (savedLessonsRef.current[moduleId] ?? []).map((row) => row.id === lesson.id ? data as Lesson : row);
    } else {
      ...
      savedLessonsRef.current[moduleId] = [...(savedLessonsRef.current[moduleId] ?? []), data as Lesson];
    }
  ...
  async function deleteLesson(moduleId: string, lessonId: string) {
    ...
    try {
      ...
      savedLessonsRef.current[moduleId] = (savedLessonsRef.current[moduleId] ?? []).filter((lesson) => lesson.id !== lessonId);
      router.refresh();
    } catch (error) { showError(error, "courses.lesson-delete"); } finally { setWorking(null); }
  }

  function markDirty(courseId: string) {
    setDirtyCourses((current) => new Set(current).add(courseId));
  }

  function courseIdForModule(moduleId: string) {
    return Object.entries(modulesByCourse).find(([, modules]) => modules.some((module) => module.id === moduleId))?.[0];
  }
  ...
  function moveLesson(moduleId: string, index: number, direction: -1 | 1) {
    ...
    if (courseId) markDirty(courseId);
  }

  function stageModules(courseId: string, rows: CourseModule[]) {
    const positioned = rows.map((module, position) => ({ ...module, position }));
    setModulesByCourse((current) => ({ ...current, [courseId]: positioned }));
    markDirty(courseId);
  }

  function moveModule(courseId: string, index: number, direction: -1 | 1) {
    const rows = [...(modulesByCourse[courseId] ?? [])];
    const target = index + direction;
    if (!rows[index] || !rows[target]) return;
    [rows[index], rows[target]] = [rows[target], rows[index]];
    stageModules(courseId, rows);
  }

  function dropModule(courseId: string, targetModuleId: string) {
  ...
  function dropLesson(targetModuleId: string, targetIndex: number) {
    ...
    setDragItem(null); setDropTarget(null);
    const courseId = courseIdForModule(targetModuleId);
    if (courseId) markDirty(courseId);
  }

  async function saveBuilderChanges(courseId: string) {
    setWorking(`builder-${courseId}`);
    const modules = modulesByCourse[courseId] ?? [];
    const lessons = modules.flatMap((module) => lessonsByModule[module.id] ?? []);
    const supabase = createClient();
    try {
      const results = await Promise.all([
        ...modules.map((module, position) => supabase.from("course_modules").update({ position }).eq("id", module.id)),
        ...lessons.map((lesson) => supabase.from("lessons").update({ module_id: lesson.module_id, position: lesson.position }).eq("id", lesson.id)),
      ]);
      const failed = results.find((result) => result.error);
      if (failed?.error) throw failed.error;
      savedModulesRef.current[courseId] = structuredClone(modules);
      for (const courseModule of modules) savedLessonsRef.current[courseModule.id] = structuredClone(lessonsByModule[courseModule.id] ?? []);
      setDirtyCourses((current) => { const next = new Set(current); next.delete(courseId); return next; });
      router.refresh();
    } catch (error) { showError(error, "courses.builder-save"); } finally { setWorking(null); }
  }

  function revertBuilderChanges(courseId: string) {
    const savedModules = structuredClone(savedModulesRef.current[courseId] ?? []);
    const affectedModuleIds = new Set([...(modulesByCourse[courseId] ?? []).map((module) => module.id), ...savedModules.map((module) => module.id)]);
    setModulesByCourse((current) => ({ ...current, [courseId]: savedModules }));
    setLessonsByModule((current) => {
      const next = { ...current };
      for (const moduleId of affectedModuleIds) next[moduleId] = structuredClone(savedLessonsRef.current[moduleId] ?? []);
      return next;
    });
    setDirtyCourses((current) => { const next = new Set(current); next.delete(courseId); return next; });
    setDragItem(null); setDropTarget(null);
  ...
  return <>
    ...
    {courses.length === 0 ? <div className="card program-empty"><NavIcon name="book-open" /><h2>Build your first program</h2><p>Create a clear sequence of sections and lessons your clients can follow at their own pace.</p><button className="btn btn-accent" onClick={() => setEditingCourse(null)}>Create program</button></div> : <div className="program-list">
      {courses.map((course) => {
        ...
        const hasUnsavedChanges = dirtyCourses.has(course.id);
        return <article className={`card program-card${expanded ? " expanded" : ""}`} key={course.id}>
          ...
          {expanded && <div className="program-builder">
            ...
            {hasUnsavedChanges && <div className="program-save-bar"><div><span className="program-status-dot draft" /><span><strong>Unsaved layout changes</strong><small>Save this order or revert to the last saved version.</small></span></div><div><button className="btn btn-sm" disabled={working === `builder-${course.id}`} onClick={() => revertBuilderChanges(course.id)}>Revert</button><button className="btn btn-primary btn-sm" disabled={working === `builder-${course.id}`} onClick={() => saveBuilderChanges(course.id)}>{working === `builder-${course.id}` ? "Saving…" : "Save changes"}</button></div></div>}

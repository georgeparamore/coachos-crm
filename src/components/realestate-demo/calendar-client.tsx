...
import { EventFormModal } from "@/components/realestate-demo/event-form-modal";
...
export function CalendarClient() {
  const { events, addEvent, updateEvent, deleteEvent } = useRealEstateDemo();
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Showings and calls — Home only shows today&apos;s.</div>
        </div>
            ...
            return (
              <div key={key} className={`card rd-week-day${isSameDay(d, today) ? " rd-day-today" : ""}`} style={{ marginBottom: 0 }}>
                ...
                {dayEvents.length === 0 ? (
                  ...
                  dayEvents.map((ev) => (
                    <button key={ev.id} className="rd-week-event rd-week-event-btn" onClick={() => setEditingEvent(ev)}>
